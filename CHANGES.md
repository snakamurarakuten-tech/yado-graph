...
## タブ選択が別のタブに奪われる問題を修正
- 原因: scroll-spy(IntersectionObserver)がジャンプ中も発火し、目的地まで
  移動する途中で通過したレールを検知して setActive を呼んでいた。
  結果「離れたタブを押すと、最後に通過したタブが選択される」状態になっていた
  (近いタブなら通過数が少ないため目立たなかった)
- 対処1: isJumping フラグを追加。タップ〜スクロール停止(140ms無イベント)まで
  spy を停止。保険で1200ms後に必ず解除
- 対処2: タブ自身の中央寄せ(横スクロール)はタップ時のみに限定。
  spy 由来では動かさないようにし、スクロール中にタブ列が暴れるのを防止

## サイト診断の指摘対応(本物の指摘のみ)
### 修正したもの
- alt属性: 全画像が alt="" (空)だった → 宿名を設定(カード/JS動的カード/
  ヒーロー/詳細ギャラリー)。画像検索SEOとアクセシビリティの両面で改善
- main要素: 存在しなかった → .scroll-area を <main> に変更(メインコンテンツ領域の明示)
- .htaccess: セキュリティヘッダー(X-Content-Type-Options / X-Frame-Options /
  Referrer-Policy / Permissions-Policy / HSTS)、静的アセットのキャッシュ制御、
  gzip圧縮、HTTPS強制リダイレクトを追加
### 診断ツールの誤検知(対応不要と判断)
- HTTPS未使用 → 実際は稼働中(診断がhttpで取得した可能性)
- sitemap.xml なし → /sitemap.xml は200で動的生成されている
- レスポンシブ非対応 → メディアクエリは10個実装済み
- パンくず/FAQ構造化データなし → 詳細・エリアページに実装済み(トップには無いのが正常)
- 空アンカーテキスト63個 → カード全体を覆う透明リンクで、aria-label 設定済み(正しい実装)
- meta keywords なし → Googleは非対応。追加しない

## 実行結果を毎回通知するように変更
- fetch-hotels.php: 完了時に必ず通知(モード・所要時間・総件数・実行ログ直近40行)。
  logline がログを蓄積し、通知本文に含める
- generate-official-summary.php: 完了時に必ず通知(生成件数・累計・スキップ内訳)。
  --hotel での単体テスト時は送らない
- 従来は異常時のみ通知だったが、正常時も毎回届くようにして稼働状況を把握できるように

## パンくずリスト構造化データのエラー修正(Search Console 1,937件)
- 原因: 詳細ページのパンくずで都道府県の href が空だったため、JSON-LD の
  item(URL)が欠落。BreadcrumbList は最後の項目以外 item が必須のため、
  Google に「項目 item がありません」として無効判定されていた
- 対処1: 都道府県のパンくずに /search?pref=... のURLを付与
  (構造化データが有効になり、内部リンクとしても機能する)
- 対処2: 全項目に item を出力(最後の自ページは自身のURL)。
  Google の解釈揺れを排除し、確実に有効化
- エリアページのパンくずは元から全項目に item があり問題なし
